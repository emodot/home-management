...
select lives_ok(
  $$ select set_config('test.hid_a', (public.create_household('  Obi home  ')).id::text, true) $$,
...
select throws_ok($$ select public.create_household('   ') $$, '23514', null, 'blank household names are rejected');
...
select throws_ok($$ select public.create_household('Anon home') $$, '42501', null, 'anon cannot create households');
reset role;
